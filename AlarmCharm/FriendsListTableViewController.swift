    ...
    fileprivate func addContactToTable(_ userID: String, contact: CNContact, numLeft: Int) {
        ...
        ref.child("users").child(hashedID).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            if let snapDictionary = snapshot.value as? NSDictionary{
                if let x = snapDictionary["alarm_time"] as? Double , x != 0 {
                    ...
                    let should_be_set = snapDictionary["need_friend_to_set"] as? Bool
                    let getting_set = snapDictionary["in_process_of_being_set"] as? Bool
                    var setBy: String? = nil
                    if getting_set!{
                        friendStatus = FriendStatus.inProgress
                    } else if !should_be_set! {
                        friendStatus = FriendStatus.alreadySet
                        setBy = snapDictionary["friend_who_set_alarm"] as? String
                    } else {
                        friendStatus = FriendStatus.needsToBeSet
                    }
                    
                    let message = snapDictionary["user_message"] as? String
